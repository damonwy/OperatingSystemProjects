...
VMPool * PageTable::vmpool_head = 0;
...
void PageTable::handle_fault(REGS * _r)
{
   ...
   // Check with registered pools to see if the addr is legitimate
   VMPool * p = vmpool_head;
   bool isValid = false;
   while(p != 0){
        if(p->is_legitimate(error_address)){
            isValid = true;
        }
        p = p->next;
   }
   if(!isValid){
        Console::puts("The address is not legitimate\n");
        return;
   }
   ...
   {
      // Page not present
      Console::puts("invalid entry in a page directory page\n");
      unsigned long table_frame_id = kernel_mem_pool->get_frames(1);
      unsigned long * page_table = (unsigned long *)(table_frame_id * Machine::PAGE_SIZE);
   ...
   if(pt[table_id] & 1 == false) {
      Console::puts("invalid entry in a page table page\n");
...
void PageTable::free_page(unsigned long _page_no) {
   ...
   unsigned long table_id = _page_no >> 12;
   unsigned long directory_id = table_id >> 10;

   table_id &= 0x3FF;
   directory_id &= 0x3FF;
   unsigned long *pt;
   // Check if is invalid entry in a page directory page
   if(current_page_table->page_directory[directory_id] & 1 == true)
   {
      // Page is present
      pt = (unsigned long *)current_page_table->page_directory[directory_id];
      
   }else{
      return;
   }

   // Check if is invalid entry in a page table page
   if(pt[table_id] & 1 == true) {
      
      unsigned long table_frame_id = pt[table_id] / Machine::PAGE_SIZE;
      process_mem_pool->release_frames(table_frame_id);
      pt[table_id] &= 0xFFFFFFFE; // marks the page invalid
      
   }else{
      return;
   }
  write_cr3(read_cr3()); // reload the CR3 register with its current value
